...
parser.add_argument('--num_epochs', default=20, type=int, dest='num_epochs',
                    help="Reload the last saved model")
parser.add_argument('--initdata', default=0.73, type=float, dest='initdata',
...
if model_load:
    ...
    if acquire_method == 'ordered':
        acquisition_function = Acquisition(train_data, init_percent=init_percent, seed=0, 
                                           acq_mode = parameters['acqmd'], acq_method = acquire_method)
    else:
        acquisition_function = Acquisition(train_data, init_percent=init_percent, seed=0, 
...
# acquisition_strat_all = [1]*24 + [5]*10 + [0]
# acquisition_strat = acquisition_strat_all[:num_acquisitions_required]
acquisition_strat = [0.19, 0.77, 0.96, 1.92, 3.84, 3.84, 3.84, 3.84, 9.61, 9.6, 9.6, 9.6, 19.21, 19.2]
