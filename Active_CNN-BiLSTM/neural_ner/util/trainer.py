    ...
    def train_model(self, num_epochs, train_data, dev_data, test_train_data, test_data, learning_rate,
        ...
        for epoch in range(1, num_epochs+1):
            ...
            for i, index in enumerate(np.random.permutation(len(train_batches))): 
                ...
                # print(type(score.data[0]))
                # print(type(np.sum(data['wordslen'])))
                # print(np.sum(data['wordslen']))
                # print(type(data['wordslen']))
                # print('HERE!!!\n\n\n')
                loss += float(score.data[0])/float(np.sum(data['wordslen']))
                score.backward()
