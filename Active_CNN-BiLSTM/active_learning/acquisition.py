...
class Acquisition(object):
    ...
    def __init__(self, train_data, acq_mode='d', init_percent=2, seed=0, usecuda = True, acq_method = 'random'):
        self.tokenlen = sum([len(x['words']) for x in train_data])
        self.train_index = set()
        self.npr = np.random.RandomState(seed)
        self.obtain_data(train_data, acquire = init_percent, method = acq_method)
    ...
    def get_ordered(self, data, num_tokens):
        # test_indices = self.npr.permutation(len(data))
    ...
    def obtain_data(self, data, model_path=None, model_name=None, acquire=2, method='random', num_samples=100):
        ...
        if method=='random':
            ...
            if method=='ordered':
                self.get_ordered(data, num_tokens)
            elif self.acq_mode == 'd':
                if method=='mnlp':
